...
| US-01-07 | 访问搜索页 `/#/search` | 显示搜索页面 | ✅ `ci/pageLoad` |
| US-01-08 | 访问订阅管理页 `/#/feed-management` | 显示订阅管理页面 | ✅ `ci/pageLoad` |
...
| US-03-04 | 自动标记文章为已读 | 打开文章后自动标记为已读 | ✅ `ci/articleReading` |
| US-03-05 | 显示已读状态标记 | 已读文章显示"✓ Read"标记 | ✅ `ci/articleReading` |
| US-03-06 | 未读文章指示器 | 未读文章显示蓝色圆点 | ✅ `feedSubscription` |
| US-03-07 | 查看原文链接 | 显示"阅读原文"外部链接 | ✅ `ci/articleReading` |
| US-03-08 | 文章元数据显示 | 显示作者、发布时间、来源 | ✅ `ci/articleReading` |
| US-03-09 | 从文章返回到文章列表 | 点击返回按钮回到订阅源详情页 | ✅ `ci/articleReading` |
...
| ID | 场景 | 预期结果 | E2E 覆盖 |
|----|------|----------|----------|
| US-04-01 | 在文章详情页收藏文章 | 点击收藏按钮，文章被标记为收藏 | ✅ `ci/favorites` |
| US-04-02 | 取消收藏文章 | 再次点击收藏按钮，取消收藏 | ❌ 缺失 |
| US-04-03 | 查看收藏列表 | 收藏页面显示所有已收藏的文章 | ✅ `ci/favorites` |
| US-04-04 | 收藏列表按时间排序 | 最新收藏的排在前面 | ❌ 缺失 |
| US-04-05 | 从收藏列表点击文章 | 可以直接跳转到文章详情 | ✅ `ci/favorites` |
...
| ID | 场景 | 预期结果 | E2E 覆盖 |
|----|------|----------|----------|
| US-05-01 | 查看阅读历史列表 | 显示最近阅读的文章（最多50条） | ✅ `ci/history` |
| US-05-02 | 历史按阅读时间排序 | 最近阅读的排在前面 | ❌ 缺失 |
| US-05-03 | 显示阅读时间 | 每篇文章显示阅读时间 | ✅ `ci/history` |
| US-05-04 | 从历史列表点击文章 | 可以跳转到文章详情重新阅读 | ✅ `ci/history` |
...
| ID | 场景 | 预期结果 | E2E 覆盖 |
|----|------|----------|----------|
| US-06-01 | 访问搜索页面 | 显示搜索输入框 | ✅ `ci/search` |
| US-06-02 | 搜索订阅源 | 输入关键词后显示匹配的订阅源 | ✅ `ci/search` |
| US-06-03 | 搜索文章 | 输入关键词后显示匹配的文章 | ❌ 缺失 |
| US-06-04 | 无搜索结果 | 显示无结果提示 | ✅ `ci/search` |
| US-06-05 | 点击搜索结果 | 跳转到对应的订阅源或文章详情 | ✅ `ci/search` |
...
| ID | 场景 | 预期结果 | E2E 覆盖 |
|----|------|----------|----------|
| US-07-01 | 访问订阅管理页面 | 显示所有订阅源列表 | ✅ `ci/feedManagement` |
| US-07-02 | 编辑订阅源标题 | 内联编辑标题并保存 | ❌ 缺失 |
| US-07-03 | 编辑订阅源 URL | 内联编辑 URL 并保存 | ❌ 缺失 |
| US-07-04 | 删除订阅源 | 确认后删除订阅源 | ❌ 缺失 |
| US-07-05 | 取消编辑 | 恢复原始值 | ✅ `ci/feedManagement` |
| US-07-06 | 快速添加订阅源 | 浮动按钮打开添加对话框 | ✅ `ci/feedManagement` |
